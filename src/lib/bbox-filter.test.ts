...
import { boundsChanged, filterByBbox, inBbox, type LatLngBoundsLike } from "./bbox-filter";
...
describe("inBbox — single-park predicate", () => {
  ...
  it("returns false for null lat or lng (stub parks with no coords)", () => {
...
describe("filterByBbox — list filtering", () => {
  ...
  it("returns empty array when no parks fall inside", () => {
...
describe("boundsChanged — threshold for 'Search this area' button", () => {
  it("returns true when prev is null (first commit)", () => {
    expect(boundsChanged(null, VALLEY)).toBe(true);
  });

  it("returns false for identical bounds", () => {
    expect(boundsChanged(VALLEY, VALLEY)).toBe(false);
  });

  it("returns false for changes within 1% tolerance (e.g., aspect-ratio shimmer)", () => {
    // 0.4 lat span * 0.005 = 0.002 — well within tolerance
    const shifted: LatLngBoundsLike = {
      south: VALLEY.south + 0.002,
      north: VALLEY.north + 0.002,
      west: VALLEY.west + 0.002,
      east: VALLEY.east + 0.002,
    };
    expect(boundsChanged(VALLEY, shifted)).toBe(false);
  });

  it("returns true when pan exceeds 1% tolerance", () => {
    // 0.4 lat span * 0.05 = 0.02 — well above 1% tolerance
    const panned: LatLngBoundsLike = {
      south: VALLEY.south + 0.02,
      north: VALLEY.north + 0.02,
      west: VALLEY.west,
      east: VALLEY.east,
    };
    expect(boundsChanged(VALLEY, panned)).toBe(true);
  });

  it("returns true when zoom changes (different bounds extent)", () => {
    // Zoom out — same center, wider extent
    const zoomedOut: LatLngBoundsLike = { south: 40.0, west: -76.4, north: 41.2, east: -74.6 };
    expect(boundsChanged(VALLEY, zoomedOut)).toBe(true);
  });

  it("accepts a custom tolerance", () => {
    // Same shifted bounds as the 1%-tolerance test — but with a tighter 0.1% tolerance, returns true
    const shifted: LatLngBoundsLike = {
      south: VALLEY.south + 0.002,
      north: VALLEY.north + 0.002,
      west: VALLEY.west,
    ...
    expect(boundsChanged(VALLEY, shifted, 0.001)).toBe(true);
  });
});
